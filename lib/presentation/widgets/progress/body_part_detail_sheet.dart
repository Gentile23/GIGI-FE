...
class _BodyPartDetailSheetState extends State<BodyPartDetailSheet> {
  ...
  String _getPartDescription() {
    final descriptions = {
      'neck_cm':
          'Il collo riflette spesso la postura e lo sviluppo dei trapezi.',
      'shoulders_cm': 'Spalle larghe contribuiscono alla forma a V del torso.',
      'chest_cm':
          'Lo sviluppo del petto è un indicatore chiave della forza della parte superiore.',
      'back_cm': 'La schiena aiuta a monitorare ampiezza e sviluppo del torso.',
      'bicep_left_cm': 'Misura nel punto di massima contrazione.',
      'bicep_right_cm': 'Misura nel punto di massima contrazione.',
      'triceps_cm':
          'Il tricipite contribuisce alla circonferenza complessiva del braccio.',
      'waist_cm':
          'La misura della vita è l\'indicatore principale della percentuale di grasso corporeo.',
      'hips_cm':
          'I fianchi sono fondamentali per monitorare la composizione della parte inferiore.',
      'thigh_left_cm':
          'Le cosce mostrano il progresso nei grandi esercizi multi-articolari.',
      'thigh_right_cm':
          'Le cosce mostrano il progresso nei grandi esercizi multi-articolari.',
      'calf_cm':
          'I polpacci sono spesso difficili da far crescere, monitorali con costanza!',
    };
    return descriptions[widget.partId] ??
        'Monitora costantemente questa zona per vedere i tuoi progressi nel tempo.';
  }
              ...
              children: [
                ...
                IconButton(
                  ...
                  style: IconButton.styleFrom(
                    backgroundColor: CleanTheme.borderSecondary.withValues(
                      alpha: 0.5,
                    ),
                  ),
                ),
                ...
                Expanded(
                  child: _buildStatCard(
                    ...
                    widget.currentValue != null
                        ? '${_parseNum(widget.currentValue)} cm'
                        : '-',
  ...
  Future<void> _deleteMeasurement(Map<String, dynamic> measurement) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Sei sicuro di voler eliminare questa rilevazione?',
        ),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      ...
      try {
        final response = await _apiClient.delete(
          'progress/measurements/$identifier',
        );
        ...
        if (response['success'] == true) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  'Impossibile eliminare la misura al momento. Riprova più tardi.',
                ),
  ...
  Future<void> _saveMeasurement(double value) async {
    ...
    try {
      ...
      final data = {'measurement_date': date, widget.partId: value};
            ...
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (value, meta) {
                if (value.toInt() >= 0 &&
                  ...
                  final dateStr =
                      filteredHistory[value.toInt()]['measurement_date'];
